...
DEMO_DIR="$ROOT_DIR/arachna-trace-demos/jvm/demo-spring-boot"
AGENT_JAR="$ROOT_DIR/arachna-trace-agents/jvm/core/agent/target/arachna-trace-agent.jar"
...
java -jar "$COLLECTOR_JAR" \
    "config=$SCRIPT_DIR/record-collector-server/arachna-collector.cfg" \
    > /dev/null 2>&1 &
COLLECTOR_PID=$!
sleep 2
...
java -jar "$PROCESSOR_JAR" \
    "config=$SCRIPT_DIR/record-processor-server/arachna-processor.cfg" \
    &
PROCESSOR_PID=$!
sleep 2
...
cd "$DEMO_DIR"
mvn spring-boot:run \
    -Dspring-boot.run.jvmArguments="-javaagent:$AGENT_JAR=config=./arachna-agent.cfg&destination=http" \
    > /dev/null 2>&1 &
MVN_PID=$!
