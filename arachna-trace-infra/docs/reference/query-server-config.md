...
The read-only HTTP API the `arachna-trace-ui` Vue app talks to.
Translates UI requests into ClickHouse SQL; no write paths,
no agent-side state.

Config file: `server/record-query-server/arachna-query.cfg`.
Passed at startup as `java -jar record-query-server.jar config=arachna-query.cfg`.
...
Default: `8082`. The Vite dev server in `arachna-trace-ui/` proxies
...
- Set to the UI's deployed origin (e.g. `https://arachna-trace.example.com`).
- Or `*` if the API sits behind a gateway that handles CORS or
  is only reachable from trusted networks.

```properties
cors_origin=https://arachna-trace.example.com
```
