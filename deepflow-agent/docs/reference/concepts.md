...
Persisted as `deepflow.sessions` keyed by `(agent_run_id,
session_id)`.
...
executors — under one request. Persisted as `deepflow.requests`
(rolled up from `deepflow.calls` by `requests_mv`).

### Call

One instrumented method invocation, named by `call_id`.
Persisted as `deepflow.calls`, with up to four payload rows in
`deepflow.payloads` (one per `kind`).
