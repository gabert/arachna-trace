# Porting Guide — Writing a DeepFlow Agent in Your Language

**Informative.** This document walks through implementing a DeepFlow
...
the seven `X-Deepflow-*` headers (TRANSPORT.md §3). Verify records
...
- **Go** — `context.Context` is the natural carrier. Wrap your
  goroutine launch helper to attach a "deepflow context" to the
  child's `ctx`.
...
A future RFC process and a separate `deepflow-spec` repo will follow
