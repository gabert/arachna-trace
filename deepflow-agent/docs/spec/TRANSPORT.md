...
| `agent_version`| string  | yes | The agent implementation version (e.g. `"deepflow-agent/0.4.1-java"`). |
...
`X-Deepflow-Jvm-Pid`); a non-Java agent SHOULD reuse the same header
...
X-Deepflow-Agent-Run-Id:    <uuid>
X-Deepflow-Hostname:        <string>
X-Deepflow-Agent-Version:   <string>
X-Deepflow-Code-Version:    <string>      ; if known
X-Deepflow-Env:             <string>      ; if known
X-Deepflow-Jvm-Pid:         <uint64 as decimal>
X-Deepflow-Started-At-Millis: <int64 as decimal>
...
POST missing `X-Deepflow-Agent-Run-Id` — it has no way to attribute
the records.
...
`X-Deepflow-Capitalized-Words` for readability.
...
The reference topic is named **`deepflow-records`**. The topic name
...
The collector MUST copy the seven `X-Deepflow-*` HTTP headers
...
extra `X-Deepflow-*` headers added by a producer are NOT
...
```json
{
  "agentRunId": "5e3f...",
  "hostname": "host-01",
  "agentVersion": "deepflow-agent/0.4.1-java",
...
> use kebab-case with an `X-Deepflow-` prefix (e.g.
> `X-Deepflow-Agent-Run-Id`); `run.json` uses camelCase
...
- **OTLP-compat** — possible if/when DeepFlow defines a mapping to
