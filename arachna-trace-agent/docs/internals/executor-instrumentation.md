...
**Solution.** Before any instrumentation happens, `ArachnaTraceAgent` creates a
...
ByteBuddy classes (relocated to `com.github.gabert.arachna.trace.shaded`).
...
IllegalAccessError: module java.base cannot access class
    com.github.gabert.arachna.trace.agent.bootstrap.PropagatingRunnable
...
premain(agentArgs, instrumentation)
  ...
  4. ArachnaTraceAdvice.setup(new RequestRecorder( safe: RequestContext is on bootstrap;
...
`ArachnaTraceAdvice.RECORDER`) and `PropagatingRunnable` (loaded by
...
- `core/agent/.../agent/ArachnaTraceAgent.java` — `premain()`,
