...
side** of Arachna Trace — the Java agent that captures runtime data, the
...
```
-javaagent flag
  -> ArachnaTraceAgent.premain()           Load config, set up ByteBuddy
    -> AgentConfig                     Parse arachna-agent.cfg + CLI args
    -> ArachnaTraceAdvice (ByteBuddy)      Thin static facade installed on matched
...
HttpDestination
  -> POST /records                     record-collector-server (Netty)
       ...
       to record headers
  -> Kafka topic 'arachna-trace-records'
  ...
  -> ClickHouse tables 'arachna_trace.calls' + 'arachna_trace.payloads'
```

For the wire-format contract that ties producer and processor together,
see [spec/SPEC.md](../../../spec/SPEC.md). For agent-run identity carriage
...
- **The collector is a thin relay.** Netty handler, no parsing, no
  ...
  Kafka, plus copy seven `X-Arachna-Trace-*` headers across. Single
...
arachna-trace-agents/jvm/
  arachna-agent.cfg                          Reference config
...
This is the deliberate extension surface of Arachna Trace: anything that
...
(`com.github.gabert.arachna.trace.shaded.*`) precisely so it can attach
...
4. Set the matching name in `arachna-agent.cfg` (`session_resolver=…`
...
The agent must never instrument its own classes. `ArachnaTraceAgent` adds
...
- `com.github.gabert.arachna.trace.agent`
- `com.github.gabert.arachna.trace.recorder`
- `com.github.gabert.arachna.trace.codec`
- `com.github.gabert.arachna.trace.shaded`
...
`maven-shade-plugin` to `com.github.gabert.arachna.trace.shaded.*` to avoid
